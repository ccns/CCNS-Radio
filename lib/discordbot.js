...
class DiscordBot {
    ...
    return function (message) {
      ...
      switch (command) {
        ...
        case 'say':
          ...
          break
        case 'playing':
          var playing = self.playlist.getPlaying()
          if (playing) {
        ...
        case 'add':
          ...
          if (playlist_match) {
            ...
            self.playlist.newListByYoutubeId(id).then(function (list) {
              if (list !== undefined) {
            ...
            self.playlist.newSongByYoutubeId(id).then(function (song_data) {
              var msg = song_data.title + '\n' +
                '<' + song_data.url + '>'
              message.channel.send(msg)
            }).catch(function (err) {
  ...
  reactionController (messageReaction, user) {
    ...
    return function (messageReaction, user) {
      ...
      switch (emoji.name) {
        ...
        case '⏯':
          ...
          break
        case '➕':
          var volume = Number(self.playlist.getVolume())
