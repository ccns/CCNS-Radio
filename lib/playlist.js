...
class Playlist {
  ...
  async newList (song_list) {
    var song_added = []
    for (const song of song_list) {
      try {
        await this.newSong(song)
        song_added.push(song)
      }
      catch (err) {
  ...
  async newListByYoutubeId (id) {
    ...
    var song_list = await this.fetcher.fetchList(id)
    var song_added = await this.newList(song_list)
    this.dispatcher.updateList({queue: await this.list.getQueue(), history: await this.list.getHistory()})
    return song_added
  }
  ...
  async pushQueue (id) {
    ...
    if (song_data) await this.list.addToQueue(song_data)
    this.dispatcher.updateList({queue: await this.list.getQueue(), history: await this.list.getHistory()})
    return song_data
  }
