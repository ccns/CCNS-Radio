...
class Playlist {
      ...
      if (in_queue || in_history || in_playing) {
        ...
        request('https://www.googleapis.com/youtube/v3/videos?part=snippet&key=' + self.api_key + '&id=' + id, function (err, res, body) {
          ...
          if (!err && !body.error) {
            ...
            console.log('[error-info]', body.error.message)
            reject({code: 1, msg: body.error.message})
          } else {
            console.log('[error] Something bad happened.')
            reject({code: 1, msg: 'Something bad happened.'})
          }
        })
      }
    })
  ...
  newList (id) {
    ...
    return new Promise(function (resolve, reject) {
      request('https://www.googleapis.com/youtube/v3/playlistItems?part=snippet&maxResults=50&key=' + self.api_key + '&playlistId=' + id, async function (err, res, body) {
        body = JSON.parse(body)
