...
const api_key = config.get('youtube_key')
...
class Playlist {
  ...
  newSong (id, error, callback) {
    ...
    if (!in_queue && !in_history && !in_playing) {
      request('https://www.googleapis.com/youtube/v3/videos?part=snippet&key=' + api_key + '&id=' + id, function (err, res, body) {
        ...
        if (body.items.length > 0) {
          ...
          this.dispatcher.updateList({queue: self.queue, history: self.history})
          callback()
        } else {
  ...
  newList (id, error) {
    ...
    var self = this
    request('https://www.googleapis.com/youtube/v3/playlistItems?part=snippet&maxResults=50&key=' + api_key + '&playlistId=' + id, function (err, res, body) {
      var body = JSON.parse(body)

      // Recursively add songs
      function recurElseList (items) {
        if (items.length == 0) return
        else {
          var item = items[0]
          self.newSong(item.snippet.resourceId.videoId, error,
            () => recurElseList(items.slice(1)))
      ...
      try {
        ...
        this.dispatch.updateList({queue: self.queue, history: self.history, title: title})
      } catch (err) {
  ...
  nextSong () {
    var song_data
    if (this.queue.length) { song_data = this.queue.shift() } else if (this.history.length) { song_data = this.history.splice(Math.floor(Math.random() * this.history.length), 1)[0] } else if (this.playing) { song_data = this.playing }
